...
echo "curl http://documentup.com/${repo}/__recompile -# -X POST -d name=\"${title}\" -d color=\"${color} -d twitter=\"${twitter}\" -d theme=\"${theme}\" -d google_analytics=\"${ga}\" -d travis=\"${travis}\" -d issues=\"${issues}\" -A \"${agent}\"  ";
